...
function Uploader({ onChange, imageURL }: any) {
  ...
  const thumbs =
    <Thumb key={file === undefined?'':file.filename}>
      <div style={thumbInner}>
        <img src={file === undefined?'':file.preview} style={img} alt={file === undefined?'':file.filename} />
      </div>
    </Thumb>;
  ...
  useEffect(
    () => () => {
      ...
      URL.revokeObjectURL(file === undefined?'':file.preview);
    },
    [file]
  );
  ...
  return (
    <section className="container uploader">
      <Container >
        <input type='file' style={inputstyle} onChange={selectFile} />
        <UploadIcon />
        <Text>
