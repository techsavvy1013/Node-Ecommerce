...
const loadProducts = (): Product[] => {
  ...
  for(let i = 0; i < res.length; i++){
    ...
    entry.image = `${PRODUCT_IMAGE_URL}/${image}/${image}_thumb.png`;   
...
//     entry.salePrice = res[i].discount_price;
//     entry.discountInPercent = res[i].discount_percent;
//     entry.image = `http://localhost:4000/admin_productimage?folder=${image.slice(0,lastIndexOf(image,'\.'))}&filename=${image}`;   
//     entry.description = res[i].description;
//     entry.type = 'grocery';
//     entry.unit = res[i].unit;
...
//       AND product_categories.product_id = ${entry.id};`)
    
//     for(var j = 0; j < tmp.length; j++)
//     {
//       var c_entry = new Category();
//       c_entry.id = tmp[j].id;
//       c_entry.name = tmp[j].name;
//       c_entry.slug = tmp[j].slug;
//       entry.categories.push(c_entry);
//     }
//     temp.push(entry);
//   }
...
/*
   var temp:Product[] = plainToClass(Product, [
    {
      id: 1,
      name: 'Banana',
      slug: 'banana',
  ...
  // {
  //   db.query(`INSERT INTO products (id, slug, name, image, description, regular_price, discount_price, unit, inserted_at, updated_at)`
  //           + ` VALUES('${temp[i].id}', '${temp[i].slug}', '${temp[i].name}', '${temp[i].image}', '${temp[i].description}', '${temp[i].price}', '${temp[i].salePrice}',`
  //           + ` '${temp[i].unit}', NOW(), NOW())`);
