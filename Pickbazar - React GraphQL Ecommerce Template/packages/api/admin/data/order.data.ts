...
const loadOrders = (): Order[] => {
  ...
  var db = connectDB();
  var res = db.query(`SELECT * FROM get_orders`);  
  ...
  for(var i = 0; i < res.length; i++)
  {
    var entry = new Order();
    entry.id = res[i].id;
    entry.customer_id = res[i].customer_id;
    entry.creation_date = new Date(res[i].date);
    entry.amount = res[i].total;
    entry.delivery_address = res[i].deliveryAddress;
    entry.delivery_fee = res[i].deliveryFee;
    entry.status = res[i].status;
    entry.payment_method = res[i].payment_method;
    entry.contact_number = res[i].deliveryContact;
    temp.push(entry);
  }
