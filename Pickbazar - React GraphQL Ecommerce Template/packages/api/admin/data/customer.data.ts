...
import Card from "../services/user/card/card.type";
import Contact from "../services/user/contact/contact.type";
import Address from "../services/user/address/address.type";
import {split} from 'lodash';
import {AVATAR_IMAGE_URL} from '../../base_url';
...
const loadCustomers = (): Customer[] => {
  ...
  for(var i = 0; i < res.length; i++)
  {
    ...
    entry.email = res[i].email;
    entry.image = res[i].avatar==null?`${AVATAR_IMAGE_URL}/man.png`:`${AVATAR_IMAGE_URL}/${res[i].avatar}`;
    ...
    for(var j = 0; j < t1.length; j++)
    {
      ...
      card.id = t1[j];
      card.cardType = t2[j];
      card.title = res[i].name;
      card.lastFourDigit = Number(t3[j]);
      if(res[i].primary_card == card.id)
        card.type = 'primary';
      else
        card.type = 'secondary';
        entry.card.push(card);
     // console.log(card);
    }
    
    entry.creation_date = new Date(res[i].inserted_at);
    entry.has_blocked = false;
    entry.total_order = res[i].order_number;
    entry.total_order_amount = res[i].order_amount;
    temp.push(entry);
  }
